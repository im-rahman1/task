  ...
  useEffect(() => {
    ...
    for (let i = 0; i < res.length; i++) {
      if (category.length == 0) {
        category.push({
          name: !res[i].category ? "" : res[i].category.name,
          items: [
            {
              id: res[i].id,
              name: res[i].name,
        ...
        if (!res[i].category) {
          ...
          for (let j = 0; j < category.length; j++) {
            if (category[j].name == res[i].category.name) {
              category[j].items.push({
                id: res[i].id,
                name: res[i].name,
              });
              found = true;
      ...
      <Formik
        ...
        {({ values }) => (
          <Form className={styles.form}>
            <div className={styles.form__conponent1}>
              <Field
                type="text"
                name="name"
                placeholder="name"
            ...
            {items.map((item, i) => (
              <div key={i}>
                <div id="checkbox-group">{item.name}</div>
                {item.items.map((singleItem, j) => (
                  <div key={j}>
...
// const res = [
//   { name: "shirt1", category: "top" },
//   { name: "shirt2", category: "top" },
//   { name: "shirt3", category: "top" },
//   { name: "pent1", category: "bottom" },
//   { name: "pent2", category: "bottom" },
//   { name: "pent3", category: "bottom" },
//   { name: "bra1", category: "bra" },
//   { name: "bar2", category: "bra" },
//   { name: "bar3", category: "bra" },
// ];
